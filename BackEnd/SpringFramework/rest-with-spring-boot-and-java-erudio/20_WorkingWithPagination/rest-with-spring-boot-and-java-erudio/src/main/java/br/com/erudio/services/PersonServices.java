...
public class PersonServices {
    ...
    public PagedModel<EntityModel<PersonVO>> findAll(Pageable pageable, String direction) {
        logger.info("Finding all people!");
        var personPage = personRepository.findAll(pageable);
        var personVOPage = personPage.map(p -> ErudioMapper.parseObject(p, PersonVO.class));
        personVOPage.map(p -> p.add(linkTo(methodOn(PersonController.class).findById(p.getId())).withSelfRel()));
        Link link = linkTo(methodOn(PersonController.class).findAll(pageable.getPageNumber(), pageable.getPageSize(), direction)).withSelfRel();
        return pagedResourcesAssembler.toModel(personVOPage, link);
    }

    public PagedModel<EntityModel<PersonVO>> findPersonsByName(String firstName, Pageable pageable, String direction) {
        logger.info("Finding all people!");
